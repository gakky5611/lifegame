...
class MyWindow(Gtk.Window):
    ...
    def on_button_clicked1(self, *args):
        ...
        self.cells = life_set.Cell(self.column, self.row, )
...
#        self.set_size_request(640, 480)
    ...
    def draw_cell(self, ctx, x, y):
        for p in self.cells.cells:
            ctx.set_source_rgb(0, 255, 0)
            ctx.rectangle(p.x * self.L_SIZE + x, p.y * self.L_SIZE + y , self.L_SIZE - self.S_SIZE, self.L_SIZE - self.S_SIZE)
            ctx.fill()
